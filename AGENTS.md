...
### Running the Optimization Worker
Optimization runs are executed asynchronously to avoid HTTP timeouts. In production (and locally for parity), start the background worker with:
```bash
python manage.py process_optimizations --loop --interval 5
```
In Railway, this is defined as the `worker` process in the `Procfile`.
...
*   **Solver**:
    The solver service in [solver.py](file:///home/santiagob/projects/agro-planner/core/services/solver.py) uses **HiGHS** via the `appsi_highs` interface. There is no fallback solver; if HiGHS is unavailable the optimization will fail with a clear error.
