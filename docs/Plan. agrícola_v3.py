...
#archivo = r"C:\Users\marco\OneDrive\Escritorio\Agro Mariana\Input_VS.xlsx"
archivo = r'C:\Users\agust\Dropbox\Agustina Anselmino - Mariana Cóccola (1)\2026 Plan. agrícola\Input.xlsx'
...
#d = sets.iloc[0:5, 6].tolist() # Destination d
...
#d_s = sets.iloc[0:2, 7].tolist() # Short destinations
#d_l = sets.iloc[0:3, 8].tolist() # Long destinations
#print(i_ns,i_p,i_s,d_s,d_l)
...
# Distances ####################################
#dist = pd.read_excel(archivo, sheet_name='Distances', header=None, usecols="A:F")
#header_row = dist[dist.iloc[:,1] == 'D1'].index[0] 
#dist.columns = dist.iloc[header_row]
#dist = dist[header_row+1:]
#dist = dist[dist.iloc[:,0].notna()]
#dist.rename(columns={dist.columns[0]: 'J'}, inplace=True)
#dist.set_index('J', inplace=True)
#dist_dict = dist.stack(future_stack=True).to_dict()
#print(dist_dict)
...
model.i_ns = pyo.Set(
    initialize=i_ns,
    within=model.i,
    doc="Non-sequencing crops in 2 years"
)
#model.i_ns.pprint()
...
#model.d = pyo.Set(initialize=d, doc='Destinations')

#model.d_s = pyo.Set(
#    initialize=d_s,
#    within=model.d,
#    doc="Short destinations"
#)
#model.d_s.pprint()
...
model.tt = pyo.Set(
    initialize=[(t1, t2) for t1 in model.t for t2 in model.t if t1 < t2]
)
...
model.t_to_c = pyo.Param(model.t, initialize={t: c for c in model.c for t in model.tc[c]})
...
################ DEFINE PARAMETERS
model.ha = pyo.Param(model.j, initialize = ha, doc='Hectares (m2) of plot j')
model.max_m = pyo.Param(model.j, initialize = max_m, doc='Maximum quantity of main crops in plot j')
model.max_s = pyo.Param(model.j, initialize = max_s, doc='Maximum quantity of secondary crops in plot j')
model.sueloj = pyo.Param(model.j, initialize = sueloj, doc='Soil type in plot j')

model.fsp = pyo.Param(model.i, model.c, initialize = fsp_dict, doc='Future harvesting price of crop i in crop season c [USD/ha]')
model.sc = pyo.Param(model.i, model.c, initialize = sc_dict, doc='Sowing cost of crop i in crop season c [USD/ha]')
model.hc = pyo.Param(model.i, model.c, initialize = hc_dict, doc='Harvesting cost of crop i in crop season c [USD/ha]')
model.frc = pyo.Param(model.i, model.j, model.c, initialize = frc_dict, doc='Fixed rental cost in crop season c [USD]')
model.vr = pyo.Param(model.i, model.j, model.c, initialize = vr_dict, doc='Variable rental cost of crop i plot j [%]')
model.tf = pyo.Param(model.i, initialize = tf_dict, doc='Trading fee of crop i [%]')
model.scp = pyo.Param(model.i, initialize = scp_dict, doc='Share of conditioned production [%]')
model.cp = pyo.Param(model.i, model.c, initialize = cp_dict, doc='Conditioning cost [USD/ton]')
model.st = pyo.Param(model.i, initialize = st_dict, doc='Proportion of short-haul transport or bagging [%]')
model.cst = pyo.Param(model.i, model.c, initialize = cst_dict, doc='Cost of short-haul transport or bagging [USD/ton]')
model.clt = pyo.Param(model.i, model.c,initialize = clt_dict, doc='Long-haul transport cost [USD/ton]')
model.gt = pyo.Param(model.i, initialize = gt, doc='Days between sowing and harvesting of the crop i')
model.st_start = pyo.Param(model.i,  initialize = st_start, doc='Sowing window start date')
model.st_end = pyo.Param(model.i, initialize = st_end, doc ='Sowing window end date')

model.setup = pyo.Param(model.i, model.i, initialize = setup_dict, doc='Setup days required between harvest of crop i and planting of crop i')
model.ar = pyo.Param(model.i, model.i, initialize = ar_dict, doc='Prohibition of sowing a crop i two years', mutable=True)
model.sueloi = pyo.Param(model.i, model.s,  initialize = sueloi_dict, doc='Soil type suitable for crop i')
model.xh = pyo.Param(model.i, model.j, model.ch, initialize = xh_dict, doc='Crop i history in plot j in previous agricultural season ch')
model.alfa = pyo.Param(model.l, initialize = alfa_dict, doc='Seniority-based weighting factor in plot j')
model.ymax = pyo.Param(model.s, model.i, initialize = y_max_dict, doc='Maximum yield of crop i in plot j')
#model.dist = pyo.Param(model.j, model.d, initialize = dist_dict, doc='Distances between plot l and destination d')
model.red = pyo.Param(model.i, model.i, initialize = red_dict, doc='Decrease in yield when crop i is planted after crop i in agricultural season t')
...
model.PROFIT = pyo.Var(domain=Reals, initialize=0, doc='Profit [USD]')
model.REVENUES = pyo.Var(domain=NonNegativeReals, initialize=0, doc='Revenues [USD]')
model.SCOSTS = pyo.Var(domain=NonNegativeReals, initialize=0, doc='Sowing costs [USD]')
model.HCOSTS = pyo.Var(domain=NonNegativeReals, initialize=0, doc='Harvesting costs [USD]')
model.RCOSTS = pyo.Var(domain=NonNegativeReals, initialize=0, doc='Rental costs [USD]')
model.PHCOSTS = pyo.Var(domain=NonNegativeReals, initialize=0, doc='Post-harvest costs [USD]')
model.ILU = pyo.Var(domain=NonNegativeReals, initialize=0, doc='Intensity of Land Use [days]')

model.Y = pyo.Var(model.i, model.j, model.t, domain=NonNegativeReals, initialize=0, doc='Harvesting quantity [tonnes] of crop i in plot j in slot t')
model.ST = pyo.Var(model.i, model.j, model.t, domain=Reals, bounds=(-77, None), initialize=0, doc='Sowing date [day] of crop i in plot j in slot t')
model.HT = pyo.Var(model.i, model.j, model.t, domain=NonNegativeReals, initialize=0, doc='Harvest date [day] of crop i in plot j in slot t')
model.Z = pyo.Var(model.i, model.j, model.t, domain=NonNegativeReals, initialize=0, doc='Impact of the crops from the previous 3 crop seasons on the average yield of a given crop i in plot l in slot t')

##################################### BINARY VARIABLES
model.X = pyo.Var(model.i, model.j, model.t, within=pyo.Binary, initialize=0, doc='1 if crop i is sown in plot j in slot t, if not 0')
model.H = pyo.Var(model.i, model.j, model.t, model.l, within=pyo.Binary, initialize=0, doc='History of the crop i in plot j in slot t in the current campaign (l=0) or in previous campaigns (l=1,2,3)')


################ OBJECTIVE FUNCTION
# Definir PROFIT correctamente
...
#    expr=alpha * model.ILU + (1-alpha) * model.PROFIT,
    sense=pyo.maximize
)
...
# model.sueloi[i, model.sueloj[j]] -> si  model.sueloj[j]=S1 -> model.sueloi[i, "S1"]
# Para cada lote j, si el suelo de j es compatible, X=1, si NO es compatible, X=0.
# Dice lo mismo que la R2 pero evitando usar ifs y demás (por lo tanto, evita complejidad)
model.soil_compatibility = pyo.Constraint(model.i, model.j, model.t, rule=soil_compatibility)

def sowingday_lb(model,i, j, t):
    c = model.t_to_c[t]  # campaña de t
...
def sequencing(model, ib, i, j, t,tb):
    ...
    max_val =  max((pyo.value(model.st_end[i])+ pyo.value(model.gt[i])) for i in model.i) + 365 * len(model.c)
    if (ord_tb < ord_t):
...
#determina secuencias no permitidas por incompatibilidad entre fechas de siembra y cosecha
for i in model.i:
    for ib in model.i:
            if (model.st_end[i] <= model.st_start[ib] + model.gt[ib] + model.setup[ib,i]) & (model.st_end[i] + 365 <= model.st_start[ib] + model.gt[ib] + model.setup[ib,i]):
...
def sequencingNAforsoil(model, ib, i, j, t):
    ...
    except IndexError: # Va a dar IndexError cuando sea = T1
        return pyo.Constraint.Skip
...
def sequencingNA(model, i, j, t, tp):
    if i not in model.i_ns:     # solo para cultivos en i_ns (I*)
        return pyo.Constraint.Skip
    middle_slots = [tt for tt in model.t if t < tt < tp] # encontrar slots intermedios t''
...
def sequencingNA_initial(model, i, j, c, t):
    if i not in model.i_ns:  # Solo para cultivos en I*
        return pyo.Constraint.Skip
    if c != 'C1': # Solo para la primera campaña del horizonte
        return pyo.Constraint.Skip
    if t not in model.tc[c]: # Asegurar que t pertenece a T_c
        return pyo.Constraint.Skip
    ch1 = 'CH1' # Campaña histórica más reciente
    previous_slots = [tt for tt in model.tc[c] if tt < t] # Periodos anteriores dentro de la campaña c1
    return model.xh[i, j, ch1] + model.X[i, j, t] <= 1 \
           + sum(
               sum(model.X[ip, j, tt] for ip in model.i 
                   if ip != i and ip != 'BARBECHO') for tt in previous_slots) \
           + sum(
               model.xh[ip, j, ch1] for ip in model.i 
...
### Aux for next equation
model.lag = pyo.Param(model.l, initialize={
    'L0': 0,
    'L1': 1,
    'L2': 2,
    'L3': 3,
...
def history1(model, i, j, t,cb):
    c = model.t_to_c[t]  # campaña de tb
    if (model.c.ord(cb) <= model.c.ord(c)):
        lag_val=model.c.ord(c) - model.c.ord(cb)
        l=model.l.at(lag_val+1)      
        return model.H[i, j, t, l] == sum(model.X[i,j,tb] for tb in model.tc[cb] if model.t.ord(tb)<model.t.ord(t))
    else:
        return pyo.Constraint.Skip  
...
def history3(model, i, j, t, ch, l):
    c = model.t_to_c[t]  # campaña de t
    ord_c = model.c.ord(c)
    ord_ch = model.ch.ord(ch) #conjunto de campañas históricas
    lag = model.lag[l]
...
def plot_gantt(model):
    ...
    cmap = plt.cm.get_cmap('tab20', len(cultivos))
...
#opt.options['LogFile'] = 'Plan_agr.log'
